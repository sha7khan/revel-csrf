...
func IsExempted(path string) bool {
	...
	if found {
		revel.AppLog.Infog("REVEL-CSRF: Ignoring exempted route '%s'...\n", path)
		return true
	}
	...
	for _, glob := range exemptionsGlobs.list {
		...
		if found {
			revel.AppLog.Infog("REVEL-CSRF: Ignoring exempted route '%s'...", path)
			return true
		}
	}
	return false
...
func ExemptedFullPath(path string) {
	revel.AppLog.Infog("REVEL-CSRF: Adding exemption '%s'...\n", path)
...
func ExemptedGlob(path string) {
	revel.AppLog.Infog("REVEL-CSRF: Adding exemption GLOB '%s'...\n", path)
